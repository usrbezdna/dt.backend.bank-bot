...
def get_card_with_related(card_uniq_id):
    ...
    """
    return Card.objects.filter(uniq_id=card_uniq_id).select_related("corresponding_account__owner").first()
...
def get_first_card_for_account(account):
    ...
    """
    return Card.objects.filter(corresponding_account=account).select_related("corresponding_account").first()
...
async def get_card_with_account_by_account_id(context, chat_id, account_id):
    ...
    error_flag = False
    card_with_acc = (
        await Card.objects.filter(corresponding_account__uniq_id=account_id)
...
async def get_bank_requisites_for_sender(context, chat_id, user_id):
    ...
    error_flag = False
    card_with_acc_opt = (
        await Card.objects.filter(corresponding_account__owner__tlg_id=user_id)
        .select_related("corresponding_account")
        .afirst()
    )
