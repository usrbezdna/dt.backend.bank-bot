...
class TelegramPaymentHandlers:
    ...
    async def check_payable(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        if command == "/check_card":
            obj_option: Optional[Card] = await self._card_service.get_card_with_related_account_by_card_id(
                uniq_id=uniq_id
            )

        else:
            obj_option: Optional[Account] = await self._account_service.get_account_by_id(uniq_id=uniq_id)
    ...
    async def send_to(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        sender_card_with_acc_opt: Optional[
            Card
        ] = await self._card_service.get_card_with_related_account_by_account_owner_id(tlg_id=user_id)
        ...
        try:
            await self._tx_service.try_transfer_to(sending_payment_account, recipient_payment_account, value)
        ...
        recipient_name_as_tuple = await self._account_service.get_owner_name_from_account_by_id(
            recipient_payment_account.uniq_id
        )
    ...
    async def list_inter(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        user_id, chat_id = update.effective_user.id, update.effective_chat.id
        usernames = await self._tx_service.get_list_of_inter_usernames(user_id)
    ...
    async def state_payable(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        if command == "/state_card":
            obj_option: Optional[Card] = await self._card_service.get_card_with_related_account_by_card_id(uniq_id)
        else:
            obj_option: Optional[Account] = await self._account_service.get_account_by_id(uniq_id)
    ...
    async def send_result_message_for_transaction_state(
        ...
        """
        transactions = await self._tx_service.get_list_of_transactions_for_the_last_month(user_id)
    ...
    async def handle_case_with_send_to_user(
        ...
        try:
            another_user: User = await self._fav_service.get_another_user(arg_user_or_id)
        ...
        account_opt = await self._account_service.get_account_by_owner(another_user)

        if account_opt is None:
            await context.bot.send_message(chat_id=chat_id, text=RSP_USER_WITH_NO_ACC)
            return None

        card_opt = await self._card_service.get_card_with_related_account_by_account_id(account_opt.uniq_id)
    ...
    async def handle_case_with_send_to_account(
        ...
        card_opt = await self._card_service.get_card_with_related_account_by_account_id(account_id)
    ...
    async def handle_case_with_send_to_card(
        ...
        card_with_acc = await self._card_service.get_card_with_related_account_by_card_id(card_id)
