...
class AccountRepository(IAccountRepository):
    def get_account_by_id(self, uniq_id: int) -> AccountSchema:
        ...
        """
        account_option = Account.objects.filter(uniq_id=uniq_id).select_related("owner").first()
    ...
    def get_full_owner_name_from_account_by_id(self, uniq_id: int) -> str:
        ...
        :param uniq_id: uniq_id of this Account
        """
        name_as_tuple_option = (
            Account.objects.values_list("owner__first_name", "owner__last_name").filter(uniq_id=uniq_id).first()
        )
