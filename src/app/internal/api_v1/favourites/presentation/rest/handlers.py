...
from app.internal.api_v1.favourites.db.exceptions import FavouriteNotFoundException, InvalidIDArgumentException, SecondTimeAdditionException, UserNotInFavouritesException
...
from app.internal.api_v1.favourites.presentation.rest.content_messages import (
    ADDED_SUCCESS, DELETED_SUCCESS, FAV_USER_NOT_FOUND, FAVS_NOT_FOUND, 
    INVALID_ID, NO_SECOND_TIME_ADDITION, NOT_IN_FAV, SELF_OPS_PROHIBITED
)
...
class RestFavouritesHandlers:
    ...
    def list_fav(self, request):
        ...
        try:
            favs_list : List[User] =  asyncio.run(self._fav_service.\
    ...
    def del_fav(self, request, argument : str | int):
        ...
        try:
            another_user : User = asyncio.run(self.\
                _fav_service.get_another_user(argument))
