...
from app.internal.api_v1.favourites.db.exceptions import (
    FavouriteNotFoundException, InvalidIDArgumentException, 
    SecondTimeAdditionException, UserNotInFavouritesException
)

from app.internal.api_v1.favourites.presentation.bot.telegram_messages import (
    ABSENT_ARG_FAV_MSG, ABSENT_FAV_USER, NOT_VALID_ID_MSG,
    ABSENT_FAV_MSG, RESTRICT_SECOND_TIME_ADD, RESTRICT_SELF_OPS, USER_NOT_IN_FAV, 
    get_result_message_for_user_favourites, get_success_msg_for_deleted_fav, get_success_msg_for_new_fav
)
...
class TelegramFavouritesHandlers:
    ...
    async def list_fav(self,  update : Update, context : ContextTypes.DEFAULT_TYPE):
        ...
        try:
            favs_list : List[User] = await self._favourite_service.\
        ...
        if len(favs_list) == 0:
            logger.info(f"User has a favorite object, but don't have any favourites users included")
            await context.bot.send_message(chat_id=chat_id, text=ABSENT_FAV_MSG)
    ...
    async def del_fav(self,  update : Update, context : ContextTypes.DEFAULT_TYPE):
        ...
        try:
            await self._favourite_service.\
                try_del_fav_from_user(user_id, another_user)
        ...
        await context.bot.send_message(
            chat_id=chat_id, text=get_success_msg_for_deleted_fav(another_user)
        )
    ...
    async def add_fav(self,  update : Update, context : ContextTypes.DEFAULT_TYPE):
        ...
        await context.bot.send_message(
            chat_id=chat_id, text=get_success_msg_for_new_fav(another_user)
        )
    ...
    async def try_get_another_user(
        ...
        """
        try:
            another_user : User = await self._favourite_service.\
                get_another_user(argument)
