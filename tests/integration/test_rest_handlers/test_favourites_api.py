...
async def test_list_favourites_endpoint_with_no_favs(get_access_token_for_user, already_verified_user, async_client):
    access_token = await get_access_token_for_user(already_verified_user)

    fav_response = await async_client.get(path="/api/favourites", headers={"Authorization": f"Bearer {access_token}"})
...
@pytest.mark.django_db(transaction=True)
@pytest.mark.error_case
async def test_list_favourites_endpoint_with_empty_favs(get_access_token_for_user, already_verified_user, async_client):
    access_token = await get_access_token_for_user(already_verified_user)
