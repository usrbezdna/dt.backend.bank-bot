...
from src.app.internal.services.favourites_service import (
    add_fav_to_user, get_list_of_favourites, 
    get_result_message_for_user_favourites,
)

from src.app.internal.transport.bot.telegram_messages import (
    ABSENT_ARG_FAV_MSG, ABSENT_FAV_MSG, 
    ABSENT_FAV_USER, ABSENT_OLD_FAV_USER, 
    NOT_VALID_ID_MSG, RESTRICT_SECOND_TIME_ADD, 
    RESTRICT_SELF_OPS, USER_NOT_IN_FAV, get_success_for_deleted_fav, 
...
async def test_list_fav_with_empty_list(
    mocked_context, 
    ...
    mocked_context.bot.send_message.assert_called_once_with(
        chat_id=telegram_chat.id, text=ABSENT_FAV_MSG
    )
...
async def test_ops_with_no_argument(
    ...
    mocked_context.bot.send_message.assert_has_calls(
        [mock_error_call] * 2, any_order=True
    )
...
async def test_ops_with_invalid_telegram_id(
    ...
    add_commands_list = [
        '/add_fav sometrashinput', '/add_fav -9999999', 
        '/add_fav -4214', '/add_fav 0'
    ]

    del_command_list = [
        '/del_fav yt3qwtfa', '/del_fav -6363214',
        '/del_fav -900', '/del_fav 0'
    ]
    ...
    mocked_context.bot.send_message.assert_has_calls(
        [mock_error_call] * updates_total_len, any_order=True
    )
...
async def test_del_fav_with_user_not_in_favs(
    ...
    mocked_context.bot.send_message.assert_called_once_with(
        chat_id=telegram_chat.id, text=USER_NOT_IN_FAV
    )
