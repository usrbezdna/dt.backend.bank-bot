...
from src.app.internal.transport.bot.telegram_messages import (
    ABSENT_ID_NUMBER, BALANCE_NOT_FOUND, 
    NOT_VALID_ID_MSG, get_message_with_balance
)
...
async def basic_test_for_payable_validations(
    ...
    mocked_context.bot.send_message.assert_has_calls(
        [mock_error_call] * len(updates_list), any_order=True
    )
...
async def test_check_payable_with_invalid_id(
    ...
    commands_list = [
        '/check_card -5215125', '/check_card text',
        '/check_account -45125', '/check_account definitely_not_an_id'
    ]

    await basic_test_for_payable_validations(
        get_list_with_updates, mocked_context, telegram_chat, 
        commands_list, NOT_VALID_ID_MSG
    )
...
async def test_check_payable_not_found_in_db(
    ...
    commands_list = [
        '/check_card 7634344', '/check_account 9783451'
    ]
     
    await basic_test_for_payable_validations(
        get_list_with_updates, mocked_context, telegram_chat, 
        commands_list, BALANCE_NOT_FOUND
    )
...
async def test_check_payable_with_valid_args(
    ...
    new_user_with_model_and_card, 
):
    
    _, account_model, card_model = await new_user_with_model_and_card(
