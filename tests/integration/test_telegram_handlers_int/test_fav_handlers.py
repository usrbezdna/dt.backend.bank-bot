...
async def test_list_fav_with_absent_fav_obj(
    telegram_fav_handlers, mocked_context, already_verified_user, telegram_chat, get_update_for_command
):
...
async def test_ops_with_invalid_telegram_id(
    telegram_fav_handlers, mocked_context, already_verified_user, telegram_chat, get_list_with_updates
):
...
async def test_ops_with_db_miss(
    ...
    mocked_context.bot.send_message.assert_has_calls([mock_add_error_call] * (add_len + del_len), any_order=True)
