...
from src.app.models import Account, Card, Transaction
...
async def basic_test_for_send_to_validations(
    ...
    for update in updates_list:
        await telegram_payment_handlers.send_to(update, mocked_context)
...
async def test_send_to_user_with_recipient_no_account(
    ...
    mocked_context.bot.send_message.assert_called_once_with(chat_id=telegram_chat.id, text=RSP_RESTRICTION)
...
async def test_send_to_with_valid_transfer(
    ...
    assert await Transaction.objects.acount() == 3

    assert (await Account.objects.aget(uniq_id=sender_account_model.uniq_id)).value == 700
