...
from src.app.models import User, Account, Card

@pytest.fixture
def new_user_with_model_and_card(
    user_model_without_verified_pn, 
    card_with_id_and_corr_account,
    account_with_id_and_owner
):
    async def inner(user_tlg_id, account_uniq_id, card_uniq_id, account_value):
        ...
        new_account_model = account_with_id_and_owner(account_uniq_id, new_user_model, account_value)
...
@pytest.fixture
def account_with_id_and_owner():
    def inner(uniq_id, owner_user_model, account_value):
        return Account(
            uniq_id = uniq_id,
            owner = owner_user_model,
            party ="PER",
            currency ="USD",
            value = account_value
        )
...
def card_with_id_and_corr_account():
    def inner(uniq_id, corr_account_model):
        return Card(
            uniq_id=uniq_id,
            corresponding_account=corr_account_model,
            expiration=datetime.date(2025, 1, 1)
        )
    return inner